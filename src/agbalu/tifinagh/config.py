"""Architecture and training configuration for the script-conversion model.

Published as `agbalu/Juba-27M`. The release name is not a code identifier (§3.9), so
...
class ModelConfig:
    """Shapes of the character-level encoder-decoder.

    The defaults are the trained release. Changing any of them produces a model whose
    `state_dict` will not load `juba_final.pt`, which is why the checkpoint carries its
    own copy and the loader reads it rather than assuming these.
    """
...
class TrainConfig:
    ...
    max_steps: int = 15_500
    seed: int = 42
    eval_interval: int = 500
    """31 validation points over the schedule. A count large enough to see where the curve
    flattens, which is the only question a first run of a new architecture answers."""
