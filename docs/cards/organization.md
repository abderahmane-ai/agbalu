...
| [**Matoub-82M**](https://huggingface.co/agbalu/Matoub-82M) | speech synthesis (preview), 82M | Speaks Kabyle at **24 kHz** — twice the incumbent `mms-tts-kab`, and Apache-2.0 where that one is non-commercial — over a **42-symbol Kabyle phoneme inventory** the base model's token table could not represent until three embedding rows were assigned and trained for `ˤ ʕ ħ`, which its own G2P would otherwise have dropped in silence. A preview: one voice published, Cycle-CER **not yet measured**. The baseline it will be measured against, **+3.56 Cycle-CER**, was itself unscored by anyone until this project measured it |
