...
| [**SiMohand-278M**](https://huggingface.co/agbalu/SiMohand-278M) | sentence embeddings and retrieval, 278M | **97.0% Recall@1** and **0.9833 MRR** finding the right Kabyle passage among 499 distractors — against **63.8%** for the multilingual backbone it was fine-tuned from, which maps five Kabyle consonants to `<unk>` and collapses the language into one cone. Its 64-dimension Matryoshka slice, a 12× compression, scores the same 97.0% |
...
| [**Matoub-82M**](https://huggingface.co/agbalu/Matoub-82M) | speech synthesis (preview), 82M | The first neural TTS model for Kabyle — StyleTTS2 fine-tune of Kokoro-82M on 21,953 restored Common Voice clips. 24 kHz, 42-symbol IPA inventory, male voice. Preview checkpoint; future production models will be released under their own dedicated names |
...
| [**KabPunct**](https://huggingface.co/datasets/agbalu/KabPunct) | punctuation restoration | 1,318,707 word-labelled sentences with per-token punctuation (`NONE`, `COMMA`, `PERIOD`, `QUESTION`, `COLON`) and casing (`LOWER`, `UPPER_INIT`) labels — the training and evaluation corpus for Belaid-31M, and the first labelled punctuation restoration corpus for any Berber language |
| [**KabG2P**](https://huggingface.co/datasets/agbalu/KabG2P) | phonetics / G2P | 25,634 Kabyle word forms with IPA transcriptions recovered at 99.53% alignment rate from 292,921 aligned tokens — zero ambiguity, 38-symbol phonetic inventory, and per-entry spirantization and vowel-backing annotations; the pronunciation dictionary for Matoub-TTS and any downstream Kabyle TTS or ASR system |
...
| voice | speaks Kabyle | **Matoub** | Lounes Matoub (1956–1998) — recorded over 500 songs in Kabyle at a time when the Algerian state was suppressing Berber language and culture, making the language audible to an entire generation. Assassinated on 25 June 1998, thirteen days after the passage of the Arabisation law he had spent years opposing. His voice is inseparable from the survival of Kabyle as a spoken language in collective memory |
| orthography standardisation | writes Kabyle correctly from any input | **Boulifa** | Si Amar ou Saïd Boulifa (1865–1931) — the first grammarian to systematically codify Kabyle Latin orthography. His 1897 *Recueil de poésies kabyles* introduced the rendering of emphatics and pharyngeals in Latin characters, solving in print what this model solves from a keyboard |
